# stage_and_build.ps1 - the write+build oracle for add_file/edit_file/add_unit: write a candidate file into
# a project's src tree, then build the WHOLE project (build_project) and report the verdict. Mostly generic;
# the only domain knob is the path guard's allowed extensions (loosened here to any file under src\ - tighten
# it to your language's source/header extensions if you want). Code on stdin (raw bytes -> BOM strip -> fence
# strip). -Proj name/path; -Path target under src\. Exits with the build's code (0 = built).
param([string] $Proj, [string] $Path)
$ErrorActionPreference = "Stop"

# Read source from stdin as raw bytes (avoid [Console]::In BOM mis-decode under a non-UTF-8 codepage).
...
$m = [regex]::Match($code, '(?s)```[a-zA-Z0-9+#._-]*\r?\n(.*?)\r?\n```')
...
# Path guard: under src\, no escape. CHANGE_ME: add an extension check (e.g. \.(cs|cpp|h)$) for your language.
