...
$onDisk = @()
if (Test-Path $srcDir) {
    Get-ChildItem -Recurse -File -Filter *.cpp $srcDir | ForEach-Object {
...
# --- one core sibling as a template (Tier 2) ---
$drv = Join-Path $srcDir "Drivers"
if (Test-Path $drv) {
    $sample = Get-ChildItem -File -Filter *.cpp $drv | Where-Object { $_.Name -notmatch '^I[A-Z]' } | Select-Object -First 1
    if (-not $sample) { $sample = Get-ChildItem -File -Filter *.cpp $drv | Select-Object -First 1 }
    if ($sample) {
        Add ("=== adapter template: srcre/" + $sample.Name + " ===")
...
if ($Include -eq "core" -or $Include -eq "all") {
    ...
    if (Test-Path $coreDir) {
        Get-ChildItem -Recurse -File -Filter *.cpp $coreDir | ForEach-Object {
            $rel = $_.FullName.Substring($rootFull.Length).TrimStart('\','/') -replace '\\','/'
