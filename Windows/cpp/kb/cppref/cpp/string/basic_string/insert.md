...
11) Implicitly converts t to a string view sv as if by std::basic_string_view<CharT, Traits> sv = t;, then inserts, before the element (if any) pointed by index, the characters from the subview [t_index, t_index + count) of sv.
...
position of the first character in str to insert
...
position of the first character in t to insert
...
overload (8) referred to a non-existing overload

refers to overload (4) correctly
...
overload (10) caused ambiguity in some cases
