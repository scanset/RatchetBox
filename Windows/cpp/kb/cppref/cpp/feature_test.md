...
Conditional trivial special member functions
...
Relaxing some restrictions on constexpr functions and function templates
...
Explicit template parameter list for generic lambdas
...
Rewording inheriting constructors: New specification for inheriting constructors (DR1941 et al)
...
Attributes for structured binding
...
Structured bindings declaration as a condition
...
std::aligned_accessor: an std::mdspan accessor expressing pointer over-alignment
...
Size-feedback in the allocator interface, e.g.: std::allocator::allocate_at_least, std::allocator_traits::allocate_at_least
...
Constexpr for std::reverse_iterator, std::move_iterator, std::array and range access
...
ConstexprIterator; constexpr comparison for std::array; misc constexpr bits (std::array::fill et al.)
...
Heterogeneous erasure in associative containers and unordered associative containers
...
Heterogeneous overloads for the remaining member functions in ordered and unordered associative containers
...
Atomic lockfree integral types (std::atomic_signed_lock_free, std::atomic_unsigned_lock_free)
...
Atomic minimum/maximum (std::atomic::fetch_min, std::atomic::fetch_max, etc)
...
Fixing atomic initialization (value-initialize std::atomic by default)
...
Allow passing callable objects as non-type template arguments to std::bind_back
...
Allow passing callable objects as non-type template arguments to std::bind_front
...
Constexpr for std::to_chars and std::from_chars for integral types
...
Constexpr for mathematical functions in <cmath> and <cstdlib>
...
constexpr for exception types, e.g. std::bad_alloc, std::bad_cast etc.
...
Misc constexpr bits (std::insert_iterator et al.)
...
Constexpr for algorithms in <numeric>
...
Misc constexpr bits (std::string_view::copy)
...
Misc constexpr bits (std::tuple::operator= et al.)
...
Misc constexpr bits (std::pair::operator= et al.)
...
Constrained relational operators for std::pair, std::tuple, std::optional, and std::variant
...
Constraining std::expected equality operators
...
Ranges-aware construction and insertion for containers and strings
...
Destroying operator delete (library support)
...
Make parts of std::array freestanding
...
Freestanding facilities in <cstring>
...
Removing std::strtok from freestanding facilities
...
Make parts of std::expected freestanding
...
Freestanding facilities in <numeric> (saturation arithmetic)
...
Definition of operator new (optional in freestanding implementations)
...
Making parts of std::optional freestanding
...
Making parts of std::string_view freestanding
...
Making parts of std::variant freestanding
...
Heterogeneous comparison lookup in unordered associative containers
...
Minimal incomplete type support for std::forward_list, std::list, and std::vector
...
Integral power-of-2 operations (std::has_single_bit, std::bit_ceil, std::bit_floor, std::bit_width)
...
Pointer-interconvertibility traits: std::is_pointer_interconvertible_with_class, std::is_pointer_interconvertible_base_of
...
std::is_sufficiently_aligned: checks a pointer’s alignment precondition
...
std::is_virtual_base_of: Type trait for detecting virtual base classes
...
Checking if a union alternative is active (std::is_within_lifetime)
...
CWG issue 1776: Replacement of class objects containing reference members (std::launder)
...
Splicing maps and sets (std::map::extract, std::map::merge, insert(node_type), etc)
...
Allow passing callable objects as non-type template arguments to std::not_fn
...
std::philox_engine: Counter-based random number engine
...
Removing “poison pill” overloads in ranges::begin, ranges::end, ranges::rbegin, ranges::rend, and ranges::size
...
Making std::basic_const_iterator follow its underlying type's convertibility
...
std::ranges::find_last, std::ranges::find_last_if, and std::ranges::find_last_if_not
...
Vector API for random number generation (std::ranges::generate_random)
...
std::reference_constructs_from_temporary and std::reference_converts_from_temporary
...
Array support of std::make_shared
...
Smart pointer creation with default initialization (std::allocate_shared_for_overwrite, std::make_shared_for_overwrite, std::make_unique_for_overwrite)
...
Making parts of std::span freestanding
...
Constructing std::span from an initializer list
...
Interfacing std::stringstreams with std::string_view
...
Concatenation of std::strings and std::string_views
...
Three-way comparison (library support); adding three-way comparison to the library
...
Alias templates for transformation traits
...
Transparent operator functors (std::less<> et al)
...
Compatibility between std::tuple and tuple-like objects (std::pair, std::array, std::subrange)
...
Type traits variable templates (std::is_void_v, etc)
...
__has_cpp_attribute must expand to a non-zero value for standard attributes
