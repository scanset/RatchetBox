...
A constructor inherited from class type C that has a first parameter of type “reference to P” (including such a constructor instantiated from a template) is excluded from the set of candidate functions when constructing an object of type D if all following conditions are satisfied:
...
5) F1 and F2 are both template specializations and F1 is more specialized according to the partial ordering rules for template specializations, or, if not that,

6) F1 and F2 are non-template functions and F1 is more partial-ordering-constrained than F2:
...
10) F1 is generated from a user-defined deduction guide and F2 is not, or, if not that,

11) F1 is the copy deduction candidate and F2 is not, or, if not that,
...
- the floating-point conversion rank of FP1 is equal to the rank of FP2, and
T3 is not a floating-point type, or

- T3 is a floating-point type whose rank is not equal to the rank of FP1, or

- the floating-point conversion subrank of FP2 is greater than the subrank of T3.
...
the behavior was unspecified when the same
...
the conversion sequence from a string literal
...
it was invalid if the overload set named by F contains
...
references and pointers were handled insonsistently in
...
surrogate call functions were not added to
...
when a function template is selected as a
...
when the implicit conversions for arguments are equally
...
overload resolution based on size of arrays was not specified
...
the determination of the candidate functions when
...
qualification conversion was checked before reference
...
a non-explicit user-defined conversion function declared with
...
same-type list-initialization of
...
conversion from enum to its underlying type
...
the set of member candidates of a unary operator @
...
when a built-in candidate is selected by overload resolution,
...
ill-formed synthesized function template specializations could
...
due to the resolution of CWG issue 1467
...
initializer list constructors lost to copy
...
there was no tiebreaker between
...
built-in candidates with the same parameter
...
when a built-in assignment operator is considered,
...
the conversion restriction regarding designated initializer
...
the explicit object paramaeter was included
...
the overload resolution for default-initialization in the context
...
the candidate set of reference initialization by conversion
...
rewritten candidates based on operator== are added
