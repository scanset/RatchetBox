...
6) Every value computation and side effect of the first (left) argument of the built-in logical AND operator &&, the built-in logical OR operator | and the built-in comma operator , is sequenced before every value computation and side effect of the second (right) argument.
...
The rule 10 has one exception: function calls made by a standard library algorithm executing under std::execution::par_unseq execution policy are unsequenced and may be arbitrarily interleaved with each other.
(since C++17)
...
sequencing of the destruction of automatic
...
“sequenced after” was used but not defined in the C++ standard
...
side effects and value computations involving a memory
...
the cases involving undefined behaviors did not consider bit-fields
