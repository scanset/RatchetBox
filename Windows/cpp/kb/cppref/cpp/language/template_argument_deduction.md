...
4) If P is an rvalue reference to a cv-unqualified template parameter (so-called forwarding references), and the corresponding function call argument is an lvalue, the type lvalue reference to A is used in place of A for deduction (Note: this is the basis for the action of std::forward. Note: in class template argument deduction, template parameter of a class template is never a forwarding reference(since C++17)):
template<class T>
...
whether array bounds would be deduced was not specified
...
deduction took place for function parameters of form
...
type parameters of reference types were not
...
in the deduction for conversion operator templates,
...
the expression of a decltype-specifier was not a non-deduced context
...
effect of implicit conversions of the arguments
...
cannot deduce array bound and element type from a braced-init-list
...
deducing an operator with non-class
...
deducing a reference non-type parameter did not
...
direct-list-initialization of auto deduces std::initializer_list

ill-formed for more than one
...
value in a noexcept specifier of a function type was not deducible
