...
- E has a single non-explicit(since C++11) user-defined conversion function to an allowable type.
...
the behavior of pointer-to-member conversions was unclear
...
enumeration type was promoted based on its underlying type

based on its value range instead
...
the conversion from double* const (*p)[3]
...
null pointer values were not guaranteed to be
...
the behavior of lvalue to rvalue conversion of
...
std::nullptr_t was convertible to bool
...
a name expression that appears in a potentially-evaluated
...
std::nullptr_t to bool was considered an implicit
...
the behavior of reading from an indeterminate
...
contextual conversions considered explicit conversion functions
...
it was unclear whether lvalue-to-rvalue conversions from
...
for derived-to-base pointer conversions and
...
char8_t and char16_t had different integral
...
integral promotions involving bit-fields were not specified well
...
temporary materialization would occur when an explicit
...
a pointer to a type-inaccessible object could be
...
temporary materialization conversion was applied on prvalue
...
lvalue-to-rvalue conversions could be applied to lvalues
...
the result of lvalue-to-rvalue conversion from an unsigned int
