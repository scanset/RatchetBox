...
The standard encourages, but doesn't require that the main thread and the threads started by std::thread and std::jthread(since C++20) offer concurrent forward progress guarantee.
...
two expression evaluations that start/end the lifetimes
...
it was unclear whether the container data race
...
the behavior of executing “trivial”[1]
infinite loops was undefined

properly defines “trivial infinite loops”
and made the behavior well-defined

- ↑ “Trivial” here means executing the infinite loop never makes any progress.