...
- template non-type arguments of integral or enumeration type
...
the this pointer, except in a constexpr function that is being evaluated as part of the expression, or when appearing in an implicit or explicit class member access expression

N2235

a control flow that passes through a declaration of a block variable with static or thread storage duration that is not usable in constant expressions
...
- an expression whose evaluation leads to any form of core language undefined or erroneous(since C++26) behavior, except for any potential undefined behavior introduced by standard attributes.
...
- an lvalue-to-rvalue implicit conversion unless applied to...

a glvalue of type (possibly cv-qualified) std::nullptr_t

- a non-volatile literal-type glvalue that designates an object that is usable in constant expressions 
int main()
{
...
- conversion from pointer to void to a pointer-to-object type T* unless the pointer holds a null pointer value or points to an object whose type is similar to T(since C++26)

- dynamic_cast whose operand is a glvalue that refers to an object whose dynamic type is constexpr-unknown(since C++20)
...
- a typeid expression applied to a glvalue of polymorphic type and that glvalue refers to an object whose dynamic type is constexpr-unknown(since C++20)

- a new expression, unless one of the following conditions is satisfied:(since C++20)
...
- a delete expression, unless it deallocates a region of storage allocated within the evaluation of this expression(since C++20)
...
- (since C++26) a construction of an exception object, unless the exception object and all of its implicit copies created by invocations of std::current_exception or std::rethrow_exception are destroyed within the evaluation of this expression
...
- a dynamic_cast or typeid expression or new expression(since C++26) that would throw an exception where no definition of the exception type is reachable(since C++26)

- inside a lambda expression, a reference to this or to a variable defined outside that lambda, if that reference would be an odr-use
void g()
{
const int n = 0;
 
...
- The full-expression of its initialization is a constant expression in the context of requiring a constant expression.
...
arithmetic constant expressions could not
...
expressions involving string literals
...
expressions involving volatile variables
could be integral constant expressions
...
it was unclear whether string literals
...
volatile glvalues could be used in constant expressions
...
reinterpret_cast is prohibited in constant expressions,
...
undefined behavior was permitted;
...
for objects that are usable in constant expressions,
...
passing constants through constexpr
...
converted constant expressions could only be prvalues
...
an address constant expression could not
...
a typeid expression whose operand is of a
...
functions needed for constant evaluation were
...
core constant expressions could evaluate any
...
binding the value of a temporary to a static storage
...
core constant expressions could invoke constexpr function template
insantiations that do not satisfy the constexpr function requirements
...
standard library undefined behaviors
...
the determination of constant expression might
...
constant initialized lifetime-extended temporaries of const-
...
integer literals were not constant expressions
...
non-member references local to an evaluation
...
it was unclear whether macros in <cstdarg>
...
invoking a constexpr virtual function on an object not usable
...
(pseudo) destructor calls lacked
...
when evaluating a core constant expression, the control
...
an indeterminate value could be a constant expression
...
variables of volatile-qualified types could be potentially-constant
...
the violation of [[noreturn]] was not required
...
converted constant expressions did
...
core constant expressions could not apply
lvalue-to-rvalue conversions to std::nullptr_t glvalues
...
a variable without an initializer could only be
...
evaluating an expression containing an identifier expression
