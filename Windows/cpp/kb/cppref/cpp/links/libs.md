...
A C/C++ package manager for Windows, Linux, and macOS.
...
The Chromaprint is an audio fingerprint library designed to identify near-identical audio. It trades precision and robustness for search performance. Chromaprint can use multiple FFT libraries - FFmpeg, FFTW3, KissFFT or vDSP. (Src)
...
The MusicBrainz Client Library (libmusicbrainz), also known as mb_client, is a development library geared towards developers who wish to add MusicBrainz lookup capabilities to their applications. The library supports Windows, Linux and Mac OS X (Src)
...
A library that handles reading and writing audio files in many common formats. (Src)
...
Single-file audio (FLAC, MP3, WAV) decoding libraries for C and C++.

Unlicense

flac

The FLAC stands for Free Lossless Audio Codec, meaning that audio compressed in FLAC has no loss in quality. FLAC stands out as the fastest and most widely supported lossless audio codec, non-proprietary, and unencumbered by patents.
...
A C library with C++ wrapper for reading and writing files containing sampled sound (e.g. WAV, AIFF) through one standard library interface. (Src)
...
An open-source, cross-platform library for reading, writing, and manipulating ID3v1 and ID3v2 tags, and retrieving some basic mp3 header info like bitrate. However, we now recommend moving to taglib :)

LGPL v2

taglib

The TagLib Audio Metadata Library is a library for reading and editing the meta-data of several popular audio formats: ID3v1, ID3v2 for MP3 files, Ogg Vorbis comments and ID3 tags and Vorbis comments in FLAC, MPC, Speex, WavPack, TrueAudio, WAV, AIFF, MP4 and ASF files. (Src)
...
A feature-rich C++ Benchmark Authoring Library/Framework. Supports Windows, Linux, and OSX using C++11.
...
A micro-benchmarking library for modern C++
...
The 'Google Performance Tools' includes a high-performance, multi-threaded malloc implementation plus tools for benchmarking heap allocation and CPU utilization.
...
A simple, fast, accurate single-header micro-benchmarking functionality for C++11/14/17/20. (Src)
...
A tiny (micro) micro-benchmarking library
...
A lowest-overhead, cross-platform simple timer class for benchmarking.
...
Fast Programmable Nexus Network. High performance fully asynchronous RPC service framework. Simultaneously supporting HTTP, WebSocket, TCP, and reliable UDP. Support the development of ultra-high load servers, with corresponding client SDKs.
...
A cross-platform (official) C++ interface for the popular GUI library GTK, including typesafe callbacks, and a comprehensive set of widgets that are easily extensible via inheritance. gtkmm uses STL, including strings, containers, and iterators. UTF8 is supported. (Src) (Doc)
...
A simple cross-platform C/C++ library for processing structured configuration files ("*.cfg") (Src) (Doc)
...
An cross-platform INI parser written in C (Doc)
...
A lightweight C++17 configuration library
...
A C++ implementation of the HOCON format developed by Pupplet.
...
A few C++20 classes for extremely fast JSON parsing/serializing
...
The most convenient C++JSON library currently in use. Read and write complex structures at any level with just one function call. The performance is approximately half that of RapidJSON.
...
TOML parser and serializer for C++17 and later

toml11

TOML parsing library based on C++11
...
Serialize objects to and from XML by adding a single, minimal, function to a class. Uses libxml++ under the hood.
...
A general purpose data serializer to serialize objects to and from XML. Uses TinyXML under the hood.
...
The libxml++ is a C++ wrapper for the libxml XML parser C library. (Doc)

LGPL-2

pugixml

A light-weight, simple and fast XML parser for C++ with XPath support
...
Another and work in progress of TinyXML.
...
A free C++ Class Library of Cryptographic Schemes. (Src)
...
A secure communications library implementing the SSL, TLS and DTLS protocols and technologies around them. (Src)
...
A robust, commercial-grade, full-featured toolkit for general-purpose cryptography and secure communication. (Src)
...
A fairly comprehensive, modular and portable cryptographic toolkit that provides developers with a vast array of well known published block ciphers, one-way hash functions, chaining modes, pseudo-random number generators, public key cryptography and a plethora of other routines. (Src)
...
An interactive C++ interpreter, built on top of Clang and LLVM to leverage RAD, creating scripts, embeddable scripting, and runtime code generation. (Src)
...
A simple to use, easy to integrate and extremely efficient run-time mathematical expression parser and evaluation engine. ExprTk supports numerous forms of functional, logical and vector processing semantics and is very easily extendible.

Jinx

A scripting language designed for video games.
...
Mozilla’s JavaScript and WebAssembly Engine.
...
A dynamic Python binding for the Qt framework. It offers an easy way to embed the Python scripting language into C++ Qt applications.

LGPL 2.1

lua

A lightweight multi-paradigm scripting language designed primarily for embedded use. C library.
...
A modern C++ library binding to Lua.
...
Binds C++ functions and classes into V8 JavaScript engine.
...
An object-oriented C++11 RTOS for microcontrollers (ARM, STM32) (Src) (Doc)
...
Embedded Template Library - C++03, Portable template library tailored for low resource (embedded) platforms (Src)

MIT

QP/C++

RTOS kernel: Real-Time Embedded Frameworks based on active objects & state machines

GPLv3

µcuREST

C++11 REST/JSON server framework for microcontrollers
...
A C++ DataFrame for statistical, Financial, and ML analysis -- in modern C++ using native types and continuous memory storage
...
A quantitative finance Library - A free/open-source library for quantitative finance
...
A fully-featured game engine and the tech behind the Frogatto & Friends.
...
A tiny library for game dev and more written in modern C++

MIT

header-only; cmake, bazel, conan, vcpkg, brew, cppget
...
A modular multi-purpose cross-platform framework
...
Simple DirectMedia Layer: a cross-platform (Windows, macOS, Linux, iOS, Android, and others), low level access to audio, keyboard, mouse, joystick, and graphics hardware via that platform's graphics API (OpenGL/Direct3D/Metal/Vulkan) (Doc) (Src)
...
Sound, Input, and Graphics Integration Library; a simple, cross-platform, minimalist library for text, shapes, input, audio, and 2D images. Supported platforms: Windows, Linux and Raspberry Pi
...
An open-source, cross-platform (Linux/Unix/BSD flavors, including Android and OS X) GPS-aware set of tools, such as a translator and replicator daemon for GPS devices, AIS radios, and other navigational sensors. gpsd is mostly written in C and Python, but also has C++ wrapper. gpsd is everywhere in mobile embedded systems. Every location-aware Android app is indirectly a gpsd client. (Doc) (Src)
...
Cross-platform C++20 modular GUI framework, with reactive capabilities, and scalable, GPU-accelerated rendering. (Src), (Doc)
...
A set of C++ libraries used to develop cross-platform software applications. It uses modern idiomatic C++ and integrates seamlessly with the STL. CopperSpice was derived from the Qt framework. (Doc) (Src)
...
A standalone C++ thread aware signal/slot library
...
A standalone C++ Unicode aware string library
...
A standalone C++ multithreading library for managing access to shared data
...
A cross-platform C++ interface for the GTK+ GUI library.
...
A small library designed to make sharing and displaying of menu structures over DBus simple and easy to use. It works for both QT and GTK+ and makes building menus simple.
...
A C++14 network/graph visualization library / Qt node editor.
...
A C++ cross-platform rapid application development framework focused on programmers productivity. It includes a set of libraries (GUI, SQL, etc.), and an integrated development environment. (Src/Bin)
...
A high-level generic library, focused on algorithms that operate on 2D images. Very limited I/O options.

BSL-1.0

agge

Anti-Grain Evolution. A 2D graphics engine for speed and quality in C++
...
A fork from the Open-Source library FreeImage v3.18 to support image codecs updates and adjust for comfortable usage. All original formats and plugins are maintained.
...
Reading, writing, and manipulating images in over 88 major formats. Forked from ImageMagick in 2002
...
Reading and writing static, animated, multi-paged images along with their meta data and ICC profiles. Converting capabilities. Targets simplicity and speed.
...
A set of C/C++ game-dev oriented libraries featuring image loader/writer/resizer, font text rasterizer, type-safe containers, ogg vorbis decoder, real-time DXT compressor, Perlin noise generator, lexer for pet DSLs, fast sprintf, and more.
...
The GNU plotutils package contains libplot, a C/C++ library for exporting 2-D vector graphics in many file formats, both vector and bitmap. libplot can animate 2-D vector graphics and uses a Postscript-like API for file export and graphics animations.

GPL

sciplot

A modern C++ scientific plotting library powered by gnuplot, with the export to PDF, SVG, PNG, EPS, etc.
...
International Components for Unicode is a mature, widely used set of C/C++ libraries providing Unicode and Globalization support for software applications. (Src)
...
A small library which parses po-files and provides an interface similar to GNU libintl. Based on boost::spirit.
...
A lightweight error-handling library for C++11: single-header format, no dependencies, no dynamic memory allocations, can be used with or without exception handling, multi-threading ready. (Src)
...
A C++11 UML state machine framework (Doc) (Src)
...
A C++ class for creating and computing arbitrary-length integers
...
A C++11 fixed-point math library that provides standard library's floating-point functionality on integers. Useful if your target platform lacks an FPU, or deterministic calculations required.
...
A C++17/20 template library providing plug-in replacements for the native arithmetic types (integer/decimal/fixed-point/floating-point/posits/logarithmic/interval)
...
A C++17 library for graph representation, manipulation, partitioning and algorithms
...
A C++14 directed graphs modelling library, part of QuickQanava project
...
Library for Efficient Modeling and Optimization in Networks implements common data structures and algorithms focusing on combinatorial optimization, graphs and networks. (Doc)
...
Open Graph algorithms and Data structures Framework - is a C++ library for graph algorithms, in particular for automatic graph drawing.
...
A C++17 based incremental decision tree
...
A C++ frontend to the popular PyTorch Python library (backend is written in C++)
...
An Open Source Machine Learning Framework for Everyone; provides stable C++ API and written mainly in C++
...
An arbitrary precision numerical calculation library developed by Bellard with a sample program that calculates π to billions of bits
...
An open-source C++ library for modeling and solving large, complicated optimization problems. It is a feature rich, mature and performant library which has been used in production at Google since 2010.
...
A C++11 library of numerical optimization methods for nonlinear functions.
...
A symbolic framework for nonlinear optimization and algorithmic differentiation. Supports C++ code generation for symbolic expressions and dynamic connection of compiled generated code.
...
A library for creating integrated systems that embed symbolic manipulations together with more established areas of computer science (like computation-intense numeric applications, graphical interfaces, etc.) under one roof.

GPLv3

mathiu.cpp

A simple computer algebra system in C++17
...
A compile-time symbolic differentiation
...
A standalone fast C++ symbolic manipulation library.
...
A symbolic math library that enables a convenient instantiation, manipulation, and evaluation of mathematical expressions at run time and compile-time.
...
A free, cross-platform, open-source library for generating PDF files. (Doc) (Src)
...
A free, open-source library for generating PDF (Doc)

MIT

PoDoFo

A free portable C++17 library to work with the PDF
...
A high performance C++ library for creating, modifying and parsing PDF files
...
A physics simulation for games, visual effects, robotics and reinforcement learning

HELICS

A co-simulation framework for synchronizing time and exchanging data between different types of simulators
...
A C++ physics engine library for 3D simulations and game
...
Sensor Fusion, SLAM, SFM, VO, Computer vision (Src)
...
Computer vision and perception, Calibration, Feature Matching (Src) (Doc)
...
A C++11 serialization

BSD

cista

A simple, high-performance, zero-copy C++ serialization & reflection library. (Src)
...
A C++11 library to encode/decode base64, base32 and hex with consistent, flexible API
...
An accurate XML serialization

iguana

A modern, universal and easy-to-use serialization engine developed in C++17, based on compile-time Reflection. Supported formats: JSON, XML, user-defined.
...
Protocol Buffers (a.k.a., protobuf) is Google's language-neutral, platform-neutral, extensible mechanism for serializing structured data, including .proto files compiler. (Doc)
...
A library that can (de)serialize types like std::map, std::vector, etc.

Serio

A fast lightweight C++ serialization library
...
A C++ serialization library for JSON
...
Yet Another Serialization is a C++11 library w/o third-party libraries dependencies. Archives can be binary, text, JSON
...
A binary serialization library designed to be compact, fast, and easy to use, itself an implementation of the same name format.

blobify

A C++17 library that infers the serialized layout from the structure definition alone.
...
A C++11 implementation of BSON format.

scons

fast_ber

A C++11 high-performance serialization using BER/DER encoding rules. Encoding layout is defined by ASN.1 schemas.

UBjsonCpp

A high-performance UBJson read-write library based on C++14
...
A function template that allows std::sort (and other random access sort functions) to be used with non-random-access containers. It also increases the performance of sorting large objects in random-access containers and arrays

zlib

Timsort

A stable sorting function template which outperforms which outperforms quicksort-based algorithms including std::sort, for reversed or semi-sorted data.
...
A C++ library for pulling system and hardware information, without hitting the command line.
...
A cross-platform library for interactive command line interfaces in modern C++.

BSL-1.0

header-only; cmake, make, nmake, vcpkg

cwidget

A high-level terminal interface library for C++, modeled on GTK+ and Qt, but using curses as its display layer
...
A readline and libedit replacement that supports UTF-8, syntax highlighting & hints.
...
Guesses a string encoding, basically the same as the uchardet function in Python.
...
A character set encoding conversion library for Linux and Mac. The Windows implementation of iconv is based on the Win32 Character Set Conversion API.
...
A companion C++ library for std::format to enable text colorization and styling.
...
An open-source formatting library providing a fast and safe alternative to C stdio and C++ iostreams. Prototype for C++20 std::format family.
...
A lightweight grammar parser library
...
A set of C++17 libraries for parsing and output generation implemented as Domain Specific Embedded Languages (DSEL) using Expression templates and Template Meta-Programming. The Spirit libraries enable a target grammar to be written exclusively in C++. (Src) (Doc)
...
A C++17 parser combinator library which allows you to write a parser by specifying it in a convenient C++ DSL with all the flexibility and control of a handwritten parser. Supports UTF-8/16/32. (Doc)
...
Parsing Expression Grammar Template Library is a zero-dependency C++17 header-only parser combinator library for creating parsers according to a Parsing Expression Grammar (PEG).
...
The CLucene is a cross-platform, full-featured, open-source indexing and searching engine/API. (Src) (Doc)
...
Ukkonen's online algorithm for constructing Suffix tree, Manber's algorithm for constructing Suffix array.
...
An open-source search engine with indexing facilities (Src) (Doc)
...
A template engine for C++17.

MIT

header-only; cmake, conan, vcpkg, etc.

Jinja2C++

A C++14/17 implementation of Jinja2 templates
...
A C++ operation library for semantic version numbers.

LibGit2

A version control system Git's core library.
...
A multi-platform, multi-architecture, open-source library which supports H.264 encoding and decoding (Src)
...
A framework for Enterprise Application Development in C++, HTTP1/HTTP2/HTTP3 compliant, supports multiple server backends
...
A C++ library for building high performance RESTful web servers. Built upon libmicrohttpd to provide a simple API for developers to create HTTP services in C++.
...
An SDK for building cross-platform C++ desktop apps with HTML/CSS/JS GUI. (Doc) (Src)
...
A powerful portable lightweight and zero-dependency web-framework for IoT and high-performance web-services.

QtWebApp

HTTP(s) Server in C++ inspired by Java Servlets

Tufão

An asynchronous web framework for C++11 built on top of Qt (Doc)
...
Widgets and building blocks for web apps, built-in security, PDF rendering, 2D and 3D painting, ORM, charting, authentication frameworks. (Doc) (Src)

GNU GPL or Commercial
...
2. 
A curated list of (awesome) header-only C++ libraries — at GitHub

3. 
A curated list of (awesome) C++ and C libraries — at GitHub

4. 
Boost C++ libraries — at Boost.org

5. 
A huge list of C++ open-source games and frameworks — at GitHub.io