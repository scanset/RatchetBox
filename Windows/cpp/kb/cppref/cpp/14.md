...
decltype(auto), Return type deduction for normal functions  (FTM)*  (FTM)*
...
Initialized/Generalized lambda captures (init-capture)  (FTM)*
...
Generic lambda expressions  (FTM)*
...
Aggregates with default member initializers  (FTM)*
...
Omitting/extending memory allocations
...
Transparent operator function objects  (FTM)*
...
Improved std::integral_constant  (FTM)*
...
User-defined literals for <chrono> and <string>  (FTM)*  (FTM)*
...
Heterogeneous associative lookup  (FTM)*
...
Addressing tuples by type: std::get<T>()  (FTM)*
...
Dual-Range std::equal, std::is_permutation, std::mismatch  (FTM)*
...
Alias templates for transformation traits  (FTM)*
...
User-defined Literals for std::complex  (FTM)*
