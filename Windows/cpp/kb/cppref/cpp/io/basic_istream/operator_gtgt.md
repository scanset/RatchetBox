...
pointer to the stream buffer to write all the data to
...
it was unclear whether overload (18) can only rethrow the
...
overload (12,13) delegated the extraction to num_get::get,
but it does not have overloads for short and int

a long value is extracted
...
overload (18) only rethrew exceptions thrown while extracting
...
overload (18) behaved as a FormattedInputFunction
...
overloads (12,13) did not store the extracted number
...
value was unchanged on extraction failure
