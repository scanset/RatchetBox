...
2) Reads successive characters from the sequence [beg, end) and parses out the time value following the same rules as the format specifier "%H:%M:%S" as used by the functions std::get_time, time_get::get, and the POSIX function strptime().
...
a stream object that this function uses to obtain locale facets when needed, e.g. std::ctype to skip whitespace
...
sets eofbit if a valid time has not been read

LWG 461

C++98

do_get_time needed to parse localized time representation

parses with "%H:%M:%S" format
