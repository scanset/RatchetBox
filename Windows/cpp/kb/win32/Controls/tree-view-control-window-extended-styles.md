...
| **TVS_EX_NOSINGLECOLLAPSE**<br> | [Windows Vista](common-control-versions.md). **Intended for internal use; not recommended for use in applications.** Do not collapse the previously selected tree-view item unless it has the same parent as the new selection. This style must be used with the [**TVS_SINGLEEXPAND**](tree-view-control-window-styles.md) style. <br> **Note:** This style may not be supported in future versions of Comctl32.dll. Also, this style is not defined in commctrl.h. Add the following definition to the source files of your application to use this style: <code>#define TVS_EX_NOSINGLECOLLAPSE 0x0001</code><br> | 
