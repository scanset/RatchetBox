...
the language linkages of the names from
...
the exception specifications of virtual
...
the specification on non-member
...
standard library functions might call non-member functions
...
<strstream> was not a C++ library header
...
C++ headers for C library facilities could
...
identifiers that are keywords or operators in C++ could
...
C++ headers must include a C++ header
...
it was unspecified whether the functions not
...
a diagnostic was required if a header
