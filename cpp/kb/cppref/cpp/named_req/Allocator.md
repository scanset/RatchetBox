...
*cp and *p identify the same object.

p->m

(as is)

Same as (*p).m, if (*p).m is well-defined.

cp->m

(as is)

Same as (*cp).m, if (*cp).m is well-defined.
...
Same as a.allocate(n), but may use cvp (nullptr or a pointer obtained from a.allocate()) in unspecified manner to aid locality.
...
Constructs an object of type X in previously-allocated storage at the address pointed to by xp, using args... as the constructor arguments.
...
Copy-constructs a1 such that a1 == a.
...
Constructs a such that it equals the prior value of A(b).
...
- (If not provided, std::allocator_traits defaults this to std::is_empty<A>::type.)
...
pointer and const_pointer were not
...
the return value of a.allocate(0) was unclear
...
T could be a const-qualified type or reference type,
...
the resolution of LWG issue 179 was accidently dropped in C++11
...
T could be a volatile-qualified object type
...
moving from an allocator might modify its value
...
allocate were not required to create an
array object in the storage it allocated

required

- ↑ The member types reference and const_reference of std::allocator are defined as T& and const T& respectively.
