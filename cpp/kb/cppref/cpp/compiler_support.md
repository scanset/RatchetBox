...
Literal suffix for (signed) size_t  (FTM)*
...
Non-literal variables (and labels and gotos) in constexpr functions  (FTM)*
...
Multidimensional subscript operator  (FTM)*
...
Named universal character escapes  (FTM)*
...
Extending the lifetime of temporaries in range-based for loop initializer  (FTM)*
...
DR98: C++ Identifier Syntax using Unicode Standard Annex 31
...
DR11: Allow duplicate attributes
...
<spanstream>: string-stream with std::span-based buffer  (FTM)*
...
constexpr for <cmath> and <cstdlib>  (FTM)*
...
Freestanding Library: Easy <utility>, <ranges>, and <iterator>
...
Formatted output library <print>  (FTM)*
...
template-parameter-list for generic lambdas  (FTM)*
...
Three-way comparison operator  (FTM)*
...
DR11: Simplifying implicit lambda capture
...
Type mismatch of defaulted special member functions
...
DR11: Specify when constexpr function definitions are needed for constant evaluation  (FTM)*
...
Pack-expansions in lambda init-captures  (FTM)*
...
Conditionally trivial special member functions  (FTM)*
...
DR17: Relaxing the structured bindings customization point finding rules
...
DR17: Allow structured bindings to accessible members
...
Destroying operator delete  (FTM)*
...
constexpr virtual function  (FTM)*
...
Immediate functions (consteval)  (FTM)*
...
Changing the active member of a union inside constexpr  (FTM)*
...
Parenthesized initialization of aggregates  (FTM)*
...
DR11: Explicitly defaulted functions with different exception specifications
...
constexpr container operations  (FTM)*
...
class template argument deduction for alias templates  (FTM)*
...
class template argument deduction for aggregates  (FTM)*
...
Inconsistencies with non-type template parameters  (FTM)*
...
DR98: Pseudo-destructors end object lifetimes
...
Extending std::make_shared() to support arrays  (FTM)*
...
Synchronized buffered (std::basic_osyncstream)  (FTM)*
...
constexpr for <algorithm> and <utility>  (FTM)*
...
More constexpr for <complex>  (FTM)*
...
Library support for operator<=> <compare>  (FTM)*
...
Utility to convert a pointer to a raw pointer  (FTM)*
...
Atomic std::shared_ptr and std::weak_ptr  (FTM)*
...
ConstexprIterator requirements  (FTM)*  (FTM)*
...
DR11: Guaranteed copy elision for piecewise construction
...
Improving the return value of erase-like algorithms  (FTM)*
...
Add std::shift_left/right to <algorithm>  (FTM)*
...
std::unwrap_ref_decay and std::unwrap_reference  (FTM)*
...
Library support for char8_t  (FTM)*
...
DR17: std::variant and std::optional should propagate copy/move triviality
...
DR17: A sane std::variant converting constructor
...
Heterogeneous lookup for unordered associative containers  (FTM)*
...
constexpr in std::pointer_traits  (FTM)*
...
Smart pointer creation with default initialization (e.g. make_unique_for_overwrite)  (FTM)*
...
Misc constexpr bits  (FTM)*  (FTM)*  (FTM)*  (FTM)*  (FTM)*  (FTM)*
...
Consistent container erasure, e.g. std::erase(std::vector), or std::erase_if(std::map)  (FTM)*
...
polymorphic_allocator<> as a vocabulary type  (FTM)*
...
DR17: Make std::create_directory() intuitive
...
std::ssize() and unsigned extent for std::span  (FTM)*
...
Traits for (un)bounded arrays  (FTM)*
...
Layout-compatibility and pointer-interconvertibility traits  (FTM)*  (FTM)*
...
std::stop_token and std::jthread  (FTM)*
...
constexpr std::allocator and related utilities  (FTM)*
...
Atomic waiting and notifying, std::counting_semaphore, std::latch and std::barrier  (FTM)*  (FTM)*  (FTM)*  (FTM)*  (FTM)*  (FTM)*
...
constexpr default constructor of std::atomic and std::atomic_flag  (FTM)*
...
constexpr for numeric algorithms  (FTM)*
...
A set of preprocessor macros to test the corresponding to C++ language and library features
