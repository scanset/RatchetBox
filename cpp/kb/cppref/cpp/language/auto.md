...
(since C++20) a concept name, optionally qualified, optionally followed by a template argument list enclosed in <>
...
- If a parameter of a function declaration has a placeholder type, an abbreviated function template is declared.
...
A placeholder type can appear in the declaration specifiers or type specifiers in the declared return type of a function declarator. Return type deduction will be applied in this case.
...
the auto specifier could be used to declare a function with a trailing
...
a parenthesized expression list could not be assigned to an auto variable
...
a declaration with the auto specifier could define two variables
...
the auto specifier in decltype(auto) was also a placeholder
...
the return type of a function pointer type-id could be auto
...
the resolution of CWG issue 1892 prohibited the deduction
