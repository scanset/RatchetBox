...
the name of the bit-field that is being declared. The name is optional: unnamed bit-fields introduce the specified number of padding bits.

size

-

an integral constant expression with a value greater or equal to zero. When greater than zero, this is the number of bits that this bit-field will occupy. The value zero is only allowed for nameless bit-fields and has special meaning.
...
it was unspecified whether the return value
...
signedness of bit-fields that are neither declared
...
unnamed bit-fields could be declared with a cv-qualified type
...
cv-qualifications were not allowed in bit-field types
