...
1) Pointer declarator: the declaration S* D; declares D as a pointer to the type determined by the declaration specifier sequence S.
...
a pointer to an object never compares equal
...
any integral constant expression that
...
the behavior of using an invalid pointer
...
the rule of composite pointer type was incomplete, and thus
...
a pointer to void and a pointer to
...
function pointer conversions were not allowed
...
reaching the end of the duration of a region
...
pointers to functions were always invalid
