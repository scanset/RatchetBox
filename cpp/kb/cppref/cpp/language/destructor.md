...
The only specifiers allowed in the declaration specifiers of a prospective(since C++20) destructor declaration areconstexpr,(since C++11) friend, inline and virtual (in particular, no return type is allowed).
...
whether automatic objects in a destructor are
...
the declarator syntax of destructor was defective (had the
...
static members might be destroyed
...
the conditions where implicitly-declared destructors are
...
the meaning of “class name” in the
...
the destructor of class X called the
destructors for X's virtual direct base classes
...
the declaration specifiers could contain consteval
