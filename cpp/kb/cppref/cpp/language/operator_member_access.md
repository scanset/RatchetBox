...
6) If E1 has a ScalarType and E2 is a ~ followed by the type name or decltype specifier designating the same type (minus cv-qualifications), optionally qualified, the result is a special kind of prvalue that can only be used as the left-hand operand of a function call operator, and for no other purpose
...
applying & to an lvalue of incomplete class type which
...
the rhs ﻿ in built-in pointer-to-member access operators could be an lvalue
...
when applying & to a non-static data member of a
...
the result of E1.E2 was unclear if E2 is a reference member or enumerator
...
if E2 is a static member function, E1.E2 is well-formed
...
the behavior of E1->E2 was unclear if E1 is a
...
E1 was not a discarded-value expression if
...
the behavior of *expr was unclear if expr
