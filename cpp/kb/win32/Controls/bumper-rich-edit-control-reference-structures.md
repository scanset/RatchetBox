# Rich Edit Structures

## In this section

-   [**BIDIOPTIONS**](/windows/desktop/api/Richedit/ns-richedit-bidioptions)
-   [**CHANGENOTIFY**](/windows/desktop/api/Textserv/ns-textserv-changenotify)
-   [**CHARFORMAT**](/windows/win32/api/richedit/ns-richedit-charformata)
-   [**CHARFORMAT2**](/windows/desktop/api/Richedit/ns-richedit-charformat2a)
-   [**CHARRANGE**](/windows/desktop/api/Richedit/ns-richedit-charrange)
-   [**CLIPBOARDFORMAT**](/windows/desktop/api/Richedit/ns-richedit-clipboardformat)
-   [**COMPCOLOR**](/windows/desktop/api/Richedit/ns-richedit-compcolor)
-   [**EDITSTREAM**](/windows/desktop/api/Richedit/ns-richedit-editstream)
-   [**ENCORRECTTEXT**](/windows/desktop/api/Richedit/ns-richedit-encorrecttext)
-   [**ENDCOMPOSITIONNOTIFY**](/windows/win32/api/richedit/ns-richedit-endcompositionnotify)
-   [**ENDROPFILES**](/windows/desktop/api/Richedit/ns-richedit-endropfiles)
-   [**ENLINK**](/windows/desktop/api/Richedit/ns-richedit-enlink)
-   [**ENLOWFIRTF**](/windows/desktop/api/Richedit/ns-richedit-enlowfirtf)
-   [**ENOLEOPFAILED**](/windows/desktop/api/Richedit/ns-richedit-enoleopfailed)
-   [**ENPROTECTED**](/windows/desktop/api/Richedit/ns-richedit-enprotected)
-   [**ENSAVECLIPBOARD**](/windows/desktop/api/Richedit/ns-richedit-ensaveclipboard)
-   [**FINDTEXT**](/windows/win32/api/richedit/ns-richedit-findtexta)
-   [**FINDTEXTEX**](/windows/desktop/api/Richedit/ns-richedit-findtextexa)
-   [**FORMATRANGE**](/windows/desktop/api/Richedit/ns-richedit-formatrange)
-   [**GETCONTEXTMENUEX**](/windows/desktop/api/Richedit/ns-richedit-getcontextmenuex)
-   [**GETTEXTEX**](/windows/desktop/api/Richedit/ns-richedit-gettextex)
-   [**GETTEXTLENGTHEX**](/windows/desktop/api/Richedit/ns-richedit-gettextlengthex)
-   [**HYPHENATEINFO**](/windows/win32/api/richedit/ns-richedit-hyphenateinfo)
-   [**HYPHRESULT**](/windows/desktop/api/Richedit/ns-richedit-hyphresult)
-   [**IMECOMPTEXT**](/windows/desktop/api/Richedit/ns-richedit-imecomptext)
-   [**MSGFILTER**](/windows/desktop/api/Richedit/ns-richedit-msgfilter)
-   [**OBJECTPOSITIONS**](/windows/desktop/api/Richedit/ns-richedit-objectpositions)
-   [**PARAFORMAT**](/windows/desktop/api/Richedit/ns-richedit-paraformat)
-   [**PARAFORMAT2**](/windows/desktop/api/Richedit/ns-richedit-paraformat2)
-   [**PUNCTUATION**](/windows/desktop/api/Richedit/ns-richedit-punctuation)
-   [**REOBJECT**](/windows/desktop/api/Richole/ns-richole-reobject)
-   [**REPASTESPECIAL**](/windows/desktop/api/Richedit/ns-richedit-repastespecial)
-   [**REQRESIZE**](/windows/desktop/api/Richedit/ns-richedit-reqresize)
-   [**RICHEDIT\_IMAGE\_PARAMETERS**](/windows/win32/api/richedit/ns-richedit-richedit_image_parameters)
-   [**SELCHANGE**](/windows/desktop/api/Richedit/ns-richedit-selchange)
-   [**SETTEXTEX**](/windows/desktop/api/Richedit/ns-richedit-settextex)
-   [**TABLECELLPARMS**](/windows/desktop/api/Richedit/ns-richedit-tablecellparms)
-   [**TABLEROWPARMS**](/windows/desktop/api/Richedit/ns-richedit-tablerowparms)
-   [**TEXTRANGE**](/windows/win32/api/richedit/ns-richedit-textrangea)

 

 



