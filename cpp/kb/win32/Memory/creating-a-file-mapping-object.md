...
The [CreateFileMapping](/windows/win32/api/WinBase/nf-winbase-createfilemappinga) function fails if the access flags conflict with those specified when the [CreateFile](/windows/win32/api/fileapi/nf-fileapi-createfilea) function opened the file. For example, to read and write to the file:
...
-   Specify the **PAGE\_READWRITE** value in the *fdwProtect* parameter of [CreateFileMapping](/windows/win32/api/WinBase/nf-winbase-createfilemappinga).
...
The *dwMaximumSizeHigh* and *dwMaximumSizeLow* parameters of [CreateFileMapping](/windows/win32/api/WinBase/nf-winbase-createfilemappinga) allow you to specify the number of bytes to be mapped from the file:
...
- The size of a file mapping object that is backed by a named file is limited by disk space. The size of a file view is limited to the largest available contiguous block of unreserved virtual memory.
