...
The [**select**](/windows/desktop/api/winsock2/nf-winsock2-select) function is used to determine the status of one or more sockets in a set. For each socket, the caller can request information on read, write, or error status. A set of sockets is indicated by an [**fd\_set**](/windows/desktop/api/winsock2/ns-winsock2-fd_set) structure.
