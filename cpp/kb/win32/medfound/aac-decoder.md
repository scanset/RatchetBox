...
| **MFAudioFormat_AAC** | Requires Windows 8. <br> This output type can be used to convert an AAC stream in the LOAS/LATM format to ADTS format. <br> To convert an LOAS/LATM stream to an ADTS stream, set the input type to **MFAudioFormat_AAC** with payload type 3 (LOAS). Then set the output type to **MFAudioFormat_AAC** with payload type 1 (ADTS). The decoder will reformat the conainter without decoding the bitstream. <br> **Note:** The decoder does not register **MFAudioFormat_AAC** as an output type. However, if the application sets the input type as described, the [**IMFTransform::GetOutputAvailableType**](/windows/desktop/api/mftransform/nf-mftransform-imftransform-getoutputavailabletype) method returns **MFAudioFormat_AAC** in the list of available output types.<br> | 
...
| [**MFT_SUPPORT_DYNAMIC_FORMAT_CHANGE**](mft-support-dynamic-format-change-attribute.md) | The AAC decoder does not handle dynamic format changes, and must be flushed or drained before a new input media type is set. Treat this attribute as read-only. <br> **Note:** The AAC decoder incorrectly reports a value of **TRUE** for this attribute.<br> In Windows 7, the decoder incorrectly reports a value of **TRUE** for this attribute. In Windows 8, the decoder reports **FALSE**, which is the correct value<br> | 
