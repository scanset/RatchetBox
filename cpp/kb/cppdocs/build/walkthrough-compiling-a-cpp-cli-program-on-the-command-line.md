...
To enable compilation for C++/CLI, you must use the [/clr](reference/clr-common-language-runtime-compilation.md) compiler option. The Microsoft C++ (MSVC) compiler generates an .exe file that contains MSIL code—or mixed MSIL and native code—and links to the required .NET Framework libraries.
...
1. On the menu bar, choose **File** > **Save**.
   ...
   You've created a C++ source file that uses a .NET Framework class (<xref:System.Console>) in the <xref:System> namespace.
