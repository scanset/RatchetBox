You are given several `.spec` files (each a short structured prompt) that together describe ONE C++
...
- `units`: every unit, in DEPENDENCY ORDER - foundation/data first, then things that depend on them.
  ...
  `.spec` filename it came from). Exactly ONE unit should have role behavior or gui as the program entry
  point - it becomes `src/main.cpp` with `int main()`. Every other unit is a component that becomes a
  `src/core/<Name>.h` declaration plus a `src/core/<Name>.cpp` definition (a normal separately-compiled
  class).
