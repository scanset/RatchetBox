# image (Go standard library)

Package image implements a basic 2-D image library.

Import path: image   Toolchain: go1.26.4
...
When operating on arbitrary images, DecodeConfig should be called before Decode,
so that the program can decide whether the image, as defined in the returned
...
image is itself malformed or not. A call to DecodeConfig which returns a header
which does not match the image returned by Decode may be considered a security
issue, and should be reported per the Go Security Policy.
