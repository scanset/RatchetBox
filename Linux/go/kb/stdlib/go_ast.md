...
Import path: go/ast   Toolchain: go1.26.4
...
func Inspect(node Node, f func(Node) bool)
    ...
    iterator over the sequence of nodes, or PreorderStack, which (like Inspect)
...
type BasicLit struct {
    ...
    For raw string literals (Kind == token.STRING && Value[0] == '`'), the Value
    field contains the string text without carriage returns (\r) that may have
    been present in the source.
...
type Directive struct {
    ...
    space-separated sequence of unquoted words, '"'-quoted Go strings,
    or '`'-quoted raw strings.
...
func ParseDirective(pos token.Pos, c string) (Directive, bool)
    ...
    used to track the position of the arguments. This may be Comment.Slash,
