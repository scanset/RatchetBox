# CQRS component: overview and wiring

The CQRS facade over watermill: wires a CommandBus, EventBus, CommandProcessor and EventProcessor on top of a Publisher/Subscriber + Router with a marshaler - so you write command/event handlers, not transport code. Keywords: watermill cqrs command query responsibility segregation event sourcing facade config commandbus eventbus processor marshaler wiring.

## source: components/cqrs/doc.go (from watermill, MIT)
...
## source: components/cqrs/cqrs.go (from watermill, MIT)
