...
The transport abstraction: Publisher.Publish(topic string, messages ...*Message) and Subscriber.Subscribe(ctx, topic) (<-chan *Message, error). Any broker (Kafka, NATS, Go channels, SQL) implements these. Keywords: watermill publisher subscriber publish subscribe topic pubsub transport broker interface channel.

## source: message/pubsub.go (from watermill, MIT)
