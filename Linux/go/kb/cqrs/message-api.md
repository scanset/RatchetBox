# Watermill Message (the event/command envelope)

The unit passed through watermill pub/sub: Message has a UUID, Payload []byte, and Metadata (string map); consumers call Ack() to confirm or Nack() to redeliver. Keywords: watermill message payload metadata ack nack uuid event command envelope publish subscribe.

## source: message/message.go (from watermill, MIT)
...
## source: message/metadata.go (from watermill, MIT)
