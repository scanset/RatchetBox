# In-memory Go-channel Pub/Sub

A Publisher+Subscriber backed by Go channels - no external broker, for tests, examples, and single-process event-driven code. Drop-in for any Publisher/Subscriber. Keywords: watermill gochannel in-memory go channel pubsub publisher subscriber test single process broker-less.

## source: pubsub/gochannel/pubsub.go (from watermill, MIT)
