...
Configure and build a concurrent cache: total capacity (max cost), a per-entry cost function, a removal/eviction listener, default TTL, and turning it into a LoadingCache with a loader function. Keywords: builder config configure capacity max cost eviction removal listener ttl loader build generic cache options.

## source: builder.go (from theine-go, MIT)
