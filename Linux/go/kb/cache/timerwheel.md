# TTL expiry via a hashed timer wheel

Expire many cache entries efficiently with a hierarchical hashed timer wheel instead of one timer per entry or scanning the whole map - the correct, scalable way to do TTL/expiration in a large cache. Keywords: ttl time to live expire expiration timer wheel hashed hierarchical timeout schedule advance deadline eviction.

## source: internal/timerwheel.go (from theine-go, MIT)
