# Production concurrent cache: public API

How to USE a high-performance concurrent cache in Go. The generic Cache[K comparable, V any] exposes Get, Set, SetWithTTL, Delete, Range, Len, Close and Stats; LoadingCache[K,V].Get(ctx, key) loads on a miss with built-in single-flight. Keywords: cache get set setwithttl delete ttl expiry generics concurrent loading cache stampede thread-safe api capacity cost.

## source: cache.go (from theine-go, MIT)
