...
A probabilistic frequency counter (count-min sketch) used by TinyLFU to approximate per-key access frequency in fixed memory, with periodic aging/halving. Keywords: count-min count min sketch frequency probabilistic estimate counter aging reset tinylfu access.

## source: internal/sketch.go (from theine-go, MIT)
