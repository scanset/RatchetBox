# Reader-biased RWMutex (read-heavy lock)

A read-write mutex optimized for read-heavy workloads (reader-biased): far cheaper reads than sync.RWMutex when many goroutines read concurrently and writes are rare - the correct lock for a high-read cache shard. To mutate you still take the write path; never mutate shared state under a read lock. Keywords: rbmutex reader biased read write mutex rwmutex rlock lock unlock contention read-heavy concurrent shard.

## source: internal/rbmutex.go (from theine-go, MIT)
